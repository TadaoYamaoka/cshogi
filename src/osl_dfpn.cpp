...
#include "osl_hash.txt"
    ...
    LibertyEstimate effective_check_short(PieceType piece_type, int dir_index, uint8_t liberty_mask);
    ...
    int dfpn_check_move_phase(const Position& pos, const Color attacker, const Move move) {
        ...
        // phase instead of falling through to Other, because OslDfPn::sort depends
    ...
    void generate_check_moves(Position& pos, std::vector<Move>& moves, bool* has_pawn_checkmate,
        ...
        if (in_check) {
            ...
            moves.reserve(static_cast<size_t>(last - buffer.data()));
            for (ExtMove* it = buffer.data(); it != last; ++it) {
                // OSL OslDfPn::generateCheck uses GenerateEscape::generateKingEscape,
        ...
        // OSL OslDfPn::generateCheck applies OslDfPn::sort directly to the
    ...
    void generate_fixed_depth_check_moves_into(Position& pos, MoveContainer& moves, bool* has_pawn_checkmate,
        ...
        // It does not call OslDfPn::generateCheck, so do not append ignored unpromotes or Dfpn-sort here.
    ...
    void generate_escape_moves(Position& pos, std::vector<Move>& moves, const bool need_full_width, const Square last_to,
        ...
        // OSL generateEscape feeds OslDfPn::sort with moves generated in
...
void OslDfPn::Impl::set_parent_link(DfpnRecord& record, const SearchContext& ctx) const {
    (void)record;
    (void)ctx;
}

Position OslDfPn::Impl::replay_position(const SearchContext& ctx, const size_t ply) const {
...
void OslDfPn::Impl::find_dag_source(Position& pos, SearchContext& ctx, DfpnRecord& terminal_record, const size_t offset, const char* call_site) {
    find_dag_source(board_index_key(pos), secondary_board_key(pos), stand_pair(pos), pos.turn(),
        ctx, terminal_record, offset, call_site);
}

void OslDfPn::Impl::find_dag_source(const Key terminal_board_index, const uint64_t terminal_board_secondary,
...
ProofDisproof OslDfPn::Impl::attack(Position& pos, SearchContext& ctx, Threshold threshold, Move* best_move, DfpnRecord* out_exact) {
    if (best_move) {
        *best_move = Move::moveNone();
    }
    ...
    ProofDisproof OslDfPn::Impl::defense(Position& pos, SearchContext& ctx, Threshold threshold, DfpnRecord* out_exact) {
    if (pos.gamePly() >= ctx.max_game_ply) {
        throw DepthLimitReached();
    }
    const Key key = pos.getKey();
...
ProofDisproof OslDfPn::Impl::try_proof(Position& pos, SearchContext& ctx, const uint32_t oracle_id, Move* best_move) {
...
ProofDisproof OslDfPn::Impl::proof_oracle_attack(Position& pos, const OracleState& oracle, SearchContext& ctx,
...
ProofDisproof OslDfPn::Impl::proof_oracle_defense(Position& pos, const OracleState& oracle, SearchContext& ctx,
    ...
    void OslDfPn::Impl::blocking_simulation(Position& pos, SearchContext& ctx, const int proof_limit, DfpnRecord& exact_record,
...
void OslDfPn::Impl::grand_parent_simulation(Position& pos, SearchContext& ctx, DfpnRecord& exact_record,
...
ProofDisproof OslDfPn::Impl::ensure_attack(Position& pos) {
...
ProofDisproof OslDfPn::Impl::ensure_defense(Position& pos) {
    const DfpnRecord entry = probe(pos, root_attack_color);
    if (entry.proof_disproof.isFinal()) {
        return entry.proof_disproof;
    }
...
bool OslDfPn::Impl::linked_pv_attack_move(Position& pos, Move& best_move, const Move incoming_move) {
    best_move = Move::moveNone();
...
bool OslDfPn::Impl::linked_pv_defense_move(Position& pos, Move& best_move, const Move incoming_move) {
    best_move = Move::moveNone();
    if (!pos.inCheck()) {
        return true;
    }
...
int OslDfPn::Impl::pv_attack(Position& pos, PvDepthTable& table, Move& best_move, const int height, const Move incoming_move,
...
int OslDfPn::Impl::pv_defense(Position& pos, PvDepthTable& table, Move& best_move, const int height, const Move incoming_move,
    const Key board_index, const uint64_t board_secondary, OslPieceNumberState* piece_numbers) {
    if (height >= max_depth) {
        best_move = Move::moveNone();
        return -1;
...
void OslDfPn::Impl::retrieve_pv(Position& pos, const bool attack_node, std::vector<u32>& pv) {
    PvDepthTable table;
    pv.clear();
...
OslDfPn::OslDfPn() : impl_(std::make_unique<Impl>()) {}

OslDfPn::OslDfPn(const OslDfPn& dfpn) : impl_(std::make_unique<Impl>()) {
    reset_config_from(dfpn);
}

OslDfPn::OslDfPn(const int max_depth, const uint32_t max_search_node, const int draw_ply)
...
OslDfPn::~OslDfPn() = default;

OslDfPn& OslDfPn::operator=(const OslDfPn& r) {
    if (this != &r) {
        reset_config_from(r);
    }
    return *this;
...
void OslDfPn::reset_config_from(const OslDfPn& other) {
...
void OslDfPn::dfpn_stop(const bool stop) {
    impl_->stop = stop;
}
...
ns_dfpn::ProbeRecord OslDfPn::dfpn_probe_record(Position& pos) {
    const DfpnRecord record = impl_->probe(pos, impl_->root_attack_color);
...
ns_dfpn::ProbeRecord OslDfPn::dfpn_probe_exact_record(Position& pos) {
...
void OslDfPn::get_pv(Position& pos, const bool is_or_node, std::vector<u32>& pv) {
    impl_->retrieve_pv(pos, is_or_node, pv);
}

int OslDfPn::pv_depth(Position& pos, const bool is_or_node) {
    OslDfPn::Impl::PvDepthTable table;
    Move best = Move::moveNone();
...
void OslDfPn::set_draw_ply(const int draw_ply) {
    impl_->draw_ply = draw_ply + 1;
}

void OslDfPn::set_maxdepth(const int depth) {
    impl_->max_depth = depth;
}

void OslDfPn::set_max_search_node(const uint32_t max_search_node) {
    impl_->max_search_node = max_search_node;
}

void OslDfPn::set_hash(const uint64_t hashMB) {
