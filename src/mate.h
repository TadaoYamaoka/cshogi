...
FORCE_INLINE bool mateMoveInEvenPly(Position& pos, const int depth, const int draw_ply = INT_MAX);
...
// 詰ます手を返すバージョン
template <bool INCHECK>
Move mateMoveInOddPlyReturnMove(Position& pos, const int depth, const int draw_ply = INT_MAX) {
	// OR節点
	...
	for (const auto& ml : ns_mate::CheckMovePicker<INCHECK>(pos, ci)) {
		...
		// 偶数手詰めチェック
		if (mateMoveInEvenPly(pos, depth - 1, draw_ply)) {
			// 詰みが見つかった時点で終了
			pos.undoMove(ml.move);
			return ml.move;
		}
...
bool mateMoveInOddPly(Position& pos, const int depth, const int draw_ply = INT_MAX)
{
	// OR節点
...
// 偶数手詰めチェック
// 手番側が王手されていること
FORCE_INLINE bool mateMoveInEvenPly(Position& pos, const int depth, const int draw_ply)
{
	// AND節点
	...
	for (Move m = move_picker.nextLegal(pos, ci.pinned); m;
		...
		if (depth == 4) {
			if (givesCheck ? !mateMoveIn3Ply<true>(pos, draw_ply) : !mateMoveIn3Ply<false>(pos, draw_ply)) {
				// 偶数手詰めでない場合
				// 詰みが見つからなかった時点で終了
				pos.undoMove(m);
				return false;
			...
			if (givesCheck || !pos.mateMoveIn1Ply()) {
				// 偶数手詰めでない場合
				// 詰みが見つからなかった時点で終了
				pos.undoMove(m);
				return false;
		...
		else if (givesCheck ? !mateMoveInOddPly<true>(pos, depth - 1, draw_ply) : !mateMoveInOddPly<false>(pos, depth - 1, draw_ply)) {
			// 偶数手詰めでない場合
			// 詰みが見つからなかった時点で終了
			pos.undoMove(m);
			return false;
